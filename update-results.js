...
//  מעדכן את Supabase, ומחשב ניקוד לכל המשתמשים (ממשחקים).
//
//  הערה: השאלות מנוהלות ידנית באפליקציה (טבלת manual_questions),
//        ולכן אין כאן יותר יצירת/עדכון שאלות אוטומטיות.
//
...
async function main() {
  ...
  // ──────────────────────────────────────────────────
  // שלב 1: שלוף תוצאות מ-API (WC)
  // ──────────────────────────────────────────────────
  var apiMatches = await fetchMatches();
  console.log('📦 נמצאו ' + apiMatches.length + ' משחקים שהסתיימו');
  ...
  //   * נקודות "שאלות בונוס" הידניות מחושבות באפליקציה
  //     מטבלת manual_questions, ולא נשמרות כאן ב-score.
  // ──────────────────────────────────────────────────
  if (updated === 0) {
  console.log('\nℹ️  אין תוצאות חדשות מ-API — אבל ממשיך לחישוב ניקוד (ייתכן עדכון ידני).');
}
  console.log('\n🔄 מחשב ניקוד מחדש...');
  ...
  for (var u = 0; u < allUsers.length; u++) {
    ...
    for (var mi = 0; mi < matchIds.length; mi++) {
      ...
      // מיפוי שלב DB ל-stage של חישוב הניקוד
